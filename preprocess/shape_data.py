...
def save_nocs_model_to_file(obj_model_dir, with_normal=False):
    ...
    for subset in ['train', 'val']:
        ...
        for synsetId in ['02876657', '02880940', '02942699', '02946921', '03642806', '03797390']:
            ...
            for instance in inst_list:
                ...
                try:
                    model_points = sample_points_from_mesh(path_to_mesh_model, 1024, with_normal, fps=True, ratio=3)
                except:
                    print(path_to_mesh_model)
                # flip z-axis in CAMERA
                model_points[:, :3] = model_points[:, :3] * np.array([[1.0, 1.0, -1.0]])
                if model_points.shape[1] == 6:
                    model_points[:, 3:] = model_points[:, 3:] * np.array([[1.0, 1.0, -1.0]])
                ...
                if synsetId == '03797390':
                    ...
                    model_points[:, :3] += shift
                    size = 2 * np.amax(np.abs(model_points[:, :3]), axis=0)
                    scale = 1 / np.linalg.norm(size)
                    model_points[:, :3] *= scale
    ...
    for subset in ['real_train', 'real_test']:
        ...
        for inst_path in inst_list:
            ...
            model_points = sample_points_from_mesh(inst_path, 1024, with_normal, fps=True, ratio=3)
            model_points[:, :3] /= scale
...
def save_model_to_hdf5(obj_model_dir, n_points, fps=False, include_distractors=False, with_normal=False):
        ...
        for catId in range(1, 7):
            ...
            for instance in inst_list:
                ...
                if catId == 6:
                    ...
                    model_points[:, :3] = scale * (model_points[:, :3] + shift)
                if subset == 'train':
    ...
    for subset in ['real_train', 'real_test']:
        ...
        for inst_path in sorted(path_to_mesh_models):
            ...
            bbox_file = inst_path.replace('.obj', '.txt')
            bbox_dims = np.loadtxt(bbox_file)
            model_points[:, :3] /= np.linalg.norm(bbox_dims)
            if catId == 6:
                shift = mug_meta[instance][0]
                scale = mug_meta[instance][1]
                model_points[:, :3] = scale * (model_points[:, :3] + shift)
...
if __name__ == '__main__':
    obj_model_dir = '../datasets/NOCS/obj_models'
    # Save ground truth models for training deform network
    save_nocs_model_to_file(obj_model_dir, with_normal=True)
    ...
    save_model_to_hdf5(obj_model_dir, n_points=2048, fps=True, with_normal=True)
