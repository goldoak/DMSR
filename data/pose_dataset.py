...
class PoseDataset(data.Dataset):
    def __init__(self, source, mode, data_dir, n_pts, img_size):
        ...
        self.dpt_dir = self.data_dir.replace('NOCS', 'dpt_output')
        self.n_pts = n_pts
        ...
        with open('assets/mean_scale.pkl', 'rb') as f:
            self.mean_scale = cPickle.load(f)
    ...
    def __getitem__(self, index):
        ...
        coord[:, :, 2] = 1 - coord[:, :, 2]
        pred_depth_path = os.path.join(self.dpt_dir, self.img_list[index] + '_depth.pkl')
        pred_normal_path = os.path.join(self.dpt_dir, self.img_list[index] + '_normal.pkl')
        ...
        pred_depth = pred_depth_all[idx]
        pred_normal = pred_normal_all[idx]
        # sample points
        ...
        pred_depth = np.concatenate([pred_depth, pred_normal], axis=0)
        ...
        gt_scale_offset = (scale - self.mean_scale[cat_id]) / self.mean_scale[cat_id]

        return pred_depth, rgb, choose, cat_id, model[:, :3], prior, sRT, nocs, gt_scale_offset, points
