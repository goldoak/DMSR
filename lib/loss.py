...
class Loss(nn.Module):
    ...
    """
    def __init__(self, corr_wt, cd_wt, entropy_wt, deform_wt, scale_wt):
    ...
    def forward(self, assign_mat, deltas, prior, nocs, model, scale_offset, gt_scale_offset):
        """
        Args:
        ...
        # L1 loss for scale offset prediction
        scale_loss = F.l1_loss(scale_offset.squeeze(), gt_scale_offset)
        scale_loss *= self.scale_wt

        # total loss
        total_loss = corr_loss + entropy_loss + cd_loss + deform_loss + scale_loss
        return total_loss, corr_loss, cd_loss, entropy_loss, deform_loss, scale_loss
...
class LowRank_Loss(nn.Module):
    ...
    def forward(self, projected_points, points):
        ...
        total_cd_loss = cd_loss1 + cd_loss2

        return total_cd_loss
